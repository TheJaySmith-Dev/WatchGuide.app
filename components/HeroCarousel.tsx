    ...
    >
      ...
      <div className="absolute inset-0 z-20 flex flex-col justify-end pb-20 px-6 md:pl-40 md:pr-24 md:pb-32 md:justify-center md:items-start">
        <div className={`transition-all duration-700 transform ${
            ...
            {logoUrl ? (
                <img src={logoUrl} alt="Logo" className="h-24 md:h-40 object-contain mb-4 md:mb-6 origin-left" />
            ) : (
                <h1 className="text-4xl md:text-6xl font-extrabold text-white mb-4 drop-shadow-lg max-w-2xl leading-tight">
                    {activeItem.title || activeItem.name}
                </h1>
            )}
        </div>

        <p className="text-gray-200 line-clamp-2 md:line-clamp-3 max-w-md md:max-w-xl text-sm md:text-lg font-light drop-shadow-md">
            {activeItem.overview}
        </p>
      </div>
    </div>
